...
The parser also extracts M-Pesa transaction amounts, charges/fees, mobile numbers, sender names, and bill/account/reference values when those fields appear in the SMS. At modem startup the app reads `txGSMSettings.PIN`, checks `AT+CPIN?`, and submits the PIN only when the modem reports `SIM PIN`; already-ready SIMs are left alone. Modem discovery scans the configured and Windows-visible COM ports at common GSM baud rates (115200, 57600, 38400, 19200, and 9600) and uses the detected rate while polling. The modem test also checks `AT+CCID` and displays `SIM detected` or `SIM not detected`. The PIN is never shown in the UI or logs. The window shows SQL connection status and modem COM port, signal strength, and network. SQL connection opening retries up to three times before reporting an error.
...
Each GSM settings row represents the machine's cashier profile. `MasterTill` identifies the till assigned to that modem, and `Entity` supplies the company heading printed above `PAYMENT APPROVED`. Set `PrintReceipt` to `1` to print every newly captured M-Pesa payment; `PrinterName` must exactly match an installed Windows printer and `Copies` controls the number of receipts. The compact receipt contains the company, approval, amount, M-Pesa transaction, customer, payment date, cashier, and receipt number. It does not print a merchant-copy heading or the raw SMS body. Paper width is detected from the Windows printer setting; put `58mm` or `80mm` in `PrinterDriver` to override detection. `PrinterPort` remains profile information, while Windows selects the actual printer by `PrinterName`. A payment is saved to SQL before printing is attempted; a printer failure is shown without losing the saved payment.

The **Messages** tab initially lists payments dated today. Use its payment-date picker to load up to 2,000 messages from another date, then select one or more rows and click **Print Selected** to print receipts manually. Manual printing uses `PrinterName`, `PrinterDriver`, and `Copies` but works when `PrintReceipt` is `0`, allowing payments to be collected without printing as they arrive.
...
The application uses SQL authentication against the server/instance configured in `appsettings.json` or the Settings tab, database `Restaurant`, with the supplied `sa` credentials in the published configuration. The database may be on another PC: enter an address such as `192.168.1.20,1433` or `SERVERNAME\INSTANCE`, then use **Test Entered Connection** before saving. Database changes apply immediately after saving. On the database PC, SQL Server must have TCP/IP enabled, listen on the selected port, allow that port through Windows Firewall, permit SQL authentication, and grant the configured login access to `Restaurant`. The default is `Server\MSSQLServer`; change the server, instance, and password to match the target SQL Server. The window and notification-area icon use M-Pesa branding; closing the window minimizes it to the tray, while `Exit` from the tray menu stops the application.
